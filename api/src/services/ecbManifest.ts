...
import ingestLogger from '../utils/ingestLogger.js';
...
export async function generateDailyManifest(opts: {
  ...
  // Financial System Safety: Explicitly check for dependency (mirror directory) availability.
  if (!fs.existsSync(mirrorDir)) {
    throw new Error(`Mirror directory not found: ${mirrorDir}. Manifest generation blocked.`);
  }
  ...
  // Resource management: explicit bounds for manifest generation
  const MAX_ENTRIES = 50000; // Maximum number of series entries in a single manifest
  const MAX_DIAGNOSTICS = 1000; // Maximum number of diagnostic records to keep in memory
  ...
  function rejectRecord(fname: string, lineNumber: number, reason: string, severity: 'warn' | 'error', rawLine?: string) {
    // Resource management: only keep a bounded number of diagnostics in memory
    ...
    try {
      ...
      for await (const raw of rl) {
        ...
        try {
          ...
          rejectRecord(fname, lineNumber, `json parse error: ${String(e?.message || e)}`, 'error', raw);
        ...
        if (!retrieved) {
          ...
          rejectRecord(fname, lineNumber, 'missing retrieved timestamp', 'warn', raw);
          continue;
        }
        ...
        if (isNaN(dt.getTime())) {
          ...
          rejectRecord(fname, lineNumber, `invalid timestamp: ${String(retrieved)}`, 'warn', raw);
          continue;
        }
        ...
        if (lineDate !== date) {
          ...
          rejectRecord(fname, lineNumber, `record date ${lineDate} does not match manifest date ${date}`, 'warn', raw);
          continue;
        }
        ...
        if (!payloadHash) {
          ...
          rejectRecord(fname, lineNumber, 'missing payload hash', 'error', raw);
          continue;
        }
        if (!/^[0-9a-f]{64}$/.test(payloadHash)) {
          // Invalid hash format -> ERROR (integrity violation)
          rejectRecord(fname, lineNumber, `invalid payload hash format: ${String(payloadHash)}`, 'error', raw);
          continue;
        }
        ...
        if (entries.length >= MAX_ENTRIES) {
          logger.error('MANIFEST_SERVICE', 'INTERNAL_SERVER_ERROR', { 
            errorCode: 'LIMIT_EXCEEDED',
            resourceId: date,
            details: { maxEntries: MAX_ENTRIES }
          });
          throw new Error(`Manifest entry limit exceeded: ${MAX_ENTRIES}`);
        }
